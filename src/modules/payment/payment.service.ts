...
export class PaymentService {
    ...
    if (fromDate || toDate) {
      ...
      if (fromDate) {
        ...
        if (isNaN(from.getTime())) throw new BadRequestException("Invalid fromDate format");
        dateFilter.$gte = from;
      }

      if (toDate) {
        const to = new Date(toDate);
        if (isNaN(to.getTime())) throw new BadRequestException("Invalid toDate format");
    ...
    return PaginationHelper.paginate(
      this.paymentModel,
      filter,
      { page, limit, sort: { createdAt: sort === 'desc' ? -1 : 1 },select: '-payUrl -transactionRef -updatedAt -isDeleted -__v' },
    );
  }
}